
import { Language } from "../types";

export const LANGUAGES: Record<Language, string> = {
  en: "English",
  hi: "हिंदी (Hindi)",
  kn: "ಕನ್ನಡ (Kannada)",
  te: "తెలుగు (Telugu)"
};

export const TRANSLATIONS = {
  en: {
    dashboard: "Dashboard",
    cropAdvisor: "Crop Advisor",
    fertilizers: "Fertilizers",
    pestControl: "Pest Control",
    chat: "Agri-Chat",
    welcome: "Welcome back,",
    farmer: "Farmer",
    yieldPred: "Yield Prediction",
    fieldReport: "View Field Report",
    totalYield: "Total Yield",
    activeFields: "Active Fields",
    equipStatus: "Equipment Status",
    waterUsage: "Water Usage",
    yieldProj: "Yield Projection",
    soilMoisture: "Soil Moisture Levels",
    searchCity: "Search city...",
    weather: "Weather",
    humidity: "Humidity",
    wind: "Wind",
    uvIndex: "UV Index",
    forecast: "5-Day Forecast",
    
    // Crop Advisor
    soilData: "Soil Data",
    enterParams: "Enter your field parameters",
    nitrogen: "Nitrogen (N)",
    phosphorus: "Phosphorus (P)",
    potassium: "Potassium (K)",
    phLevel: "pH Level",
    rainfall: "Rainfall (mm)",
    analyze: "Analyze & Recommend",
    analyzing: "Analyzing...",
    confidence: "Confidence",
    growthTime: "Growth Time",
    yield: "Yield",
    economy: "Economy",
    analysis: "Analysis",
    requirements: "Requirements",
    waterNeeds: "Water Needs",
    sunlight: "Sunlight",
    soilPref: "Soil Preference",
    clickAnalysis: "Click for Analysis",
    noCrops: "Enter soil details to get AI-powered crop advice.",

    // Fertilizer
    calcTitle: "Smart Fertilizer Calculator",
    calcDesc: "Get precise nutrient dosage plans tailored to your specific crop and soil type.",
    targetCrop: "Target Crop",
    soilCond: "Soil Condition",
    genPlan: "Generate Nutrient Plan",
    appFreq: "Application Frequency",
    dosage: "Recommended Dosage",
    warnings: "Important Warnings",
    loamy: "Loamy (Balanced)",
    clay: "Clay (Heavy)",
    sandy: "Sandy (Drains fast)",
    silt: "Silt (Retains moisture)",
    peaty: "Peaty (Acidic)",

    // Pest
    pestTitle: "AI Pest Diagnostics",
    pestDesc: "Upload a photo of an affected plant leaf or bug for instant analysis.",
    upload: "Click to upload",
    changePhoto: "Change Photo",
    diagnose: "Diagnose Problem",
    analyzingPest: "AI is examining the image...",
    treatments: "Suggested Treatments",
    prevention: "Preventive Measures",
    risk: "Risk",
    
    // Chat
    chatTitle: "Agri-Advisor",
    online: "Online",
    clearChat: "Clear Chat",
    typeMsg: "Ask about crops, pests, or farming tips...",
    suggested: "Suggested Topics",
    tip: "Today's Tip",
    // List of tips for rotation
    tipsList: [
      "Crop rotation helps return nutrients to the soil without synthetic fertilizers.",
      "Water your crops early in the morning to minimize evaporation.",
      "Mulching can suppress weeds and retain soil moisture effectively.",
      "Test your soil pH annually to ensure optimal nutrient absorption.",
      "Encourage beneficial insects like ladybugs to control pests naturally.",
      "Drip irrigation saves up to 50% water compared to flood irrigation.",
      "Composting organic waste reduces landfill use and enriches your soil."
    ],

    // Tour
    takeTour: "Take a Tour",
    tourNext: "Next",
    tourBack: "Back",
    tourSkip: "Skip",
    tourFinish: "Finish",
    tourStep1Title: "Language Selection",
    tourStep1Desc: "Start here! Change the application language to Hindi, Kannada, or Telugu for better accessibility.",
    tourStep2Title: "Dashboard",
    tourStep2Desc: "This is your home. View a summary of your farm status, quick stats, and yield projections.",
    tourStep3Title: "Weather Widget",
    tourStep3Desc: "Check real-time weather and severe alerts. Use the search bar to check forecasts for other locations.",
    tourStep4Title: "Crop Advisor",
    tourStep4Desc: "Use this before planting. Enter your soil data to get AI-recommended crops suitable for your land.",
    tourStep5Title: "Fertilizer Advisor",
    tourStep5Desc: "Use this during growth. Get custom nutrient plans to maximize yield without harming the soil.",
    tourStep6Title: "Pest Control",
    tourStep6Desc: "Use this if you see bugs or disease. Upload a photo to identify pests and get treatment advice.",
    tourStep7Title: "Agri-Chat",
    tourStep7Desc: "Have a specific question? Chat with our AI assistant for instant answers about farming.",
    tourStep8Title: "Return Home",
    tourStep8Desc: "Click here anytime to return to the main dashboard."
  },
  hi: {
    dashboard: "डैशबोर्ड",
    cropAdvisor: "फसल सलाहकार",
    fertilizers: "उर्वरक (खाद)",
    pestControl: "कीट नियंत्रण",
    chat: "कृषि-चैट",
    welcome: "वापसी पर स्वागत है,",
    farmer: "किसान",
    yieldPred: "उपज पूर्वानुमान",
    fieldReport: "खेत की रिपोर्ट देखें",
    totalYield: "कुल उपज",
    activeFields: "सक्रिय खेत",
    equipStatus: "उपकरण स्थिति",
    waterUsage: "जल उपयोग",
    yieldProj: "उपज प्रक्षेपण",
    soilMoisture: "मिट्टी की नमी का स्तर",
    searchCity: "शहर खोजें...",
    weather: "मौसम",
    humidity: "नमी",
    wind: "हवा",
    uvIndex: "यूवी सूचकांक",
    forecast: "5-दिवसीय पूर्वानुमान",

    // Crop Advisor
    soilData: "मिट्टी का डेटा",
    enterParams: "अपने खेत के विवरण दर्ज करें",
    nitrogen: "नाइट्रोजन (N)",
    phosphorus: "फॉस्फोरस (P)",
    potassium: "पोटेशियम (K)",
    phLevel: "पीएच स्तर",
    rainfall: "वर्षा (मिमी)",
    analyze: "विश्लेषण और सिफारिश",
    analyzing: "विश्लेषण कर रहा है...",
    confidence: "विश्वास",
    growthTime: "विकास समय",
    yield: "उपज",
    economy: "अर्थव्यवस्था",
    analysis: "विश्लेषण",
    requirements: "आवश्यकताएँ",
    waterNeeds: "पानी की आवश्यकता",
    sunlight: "सूर्य का प्रकाश",
    soilPref: "मिट्टी की प्राथमिकता",
    clickAnalysis: "विश्लेषण के लिए क्लिक करें",
    noCrops: "एआई-संचालित फसल सलाह प्राप्त करने के लिए मिट्टी का विवरण दर्ज करें।",

    // Fertilizer
    calcTitle: "स्मार्ट उर्वरक कैलकुलेटर",
    calcDesc: "अपनी विशिष्ट फसल और मिट्टी के प्रकार के लिए सटीक पोषक तत्व खुराक योजना प्राप्त करें।",
    targetCrop: "लक्षित फसल",
    soilCond: "मिट्टी की स्थिति",
    genPlan: "पोषक तत्व योजना बनाएं",
    appFreq: "आवेदन की आवृत्ति",
    dosage: "अनुशंसित खुराक",
    warnings: "महत्वपूर्ण चेतावनी",
    loamy: "दोमट (संतुलित)",
    clay: "चिकनी (भारी)",
    sandy: "रेतीली (तेजी से सूखने वाली)",
    silt: "गाद (नमी बनाए रखती है)",
    peaty: "पीटी (अम्लीय)",

    // Pest
    pestTitle: "एआई कीट निदान",
    pestDesc: "त्वरित विश्लेषण के लिए प्रभावित पौधे की पत्ती या कीड़े की तस्वीर अपलोड करें।",
    upload: "अपलोड करने के लिए क्लिक करें",
    changePhoto: "तस्वीर बदलें",
    diagnose: "समस्या का निदान करें",
    analyzingPest: "एआई छवि की जांच कर रहा है...",
    treatments: "सुझाए गए उपचार",
    prevention: "निवारक उपाय",
    risk: "जोखिम",

    // Chat
    chatTitle: "कृषि सलाहकार",
    online: "ऑनलाइन",
    clearChat: "चैट साफ़ करें",
    typeMsg: "फसलों, कीटों या खेती के सुझावों के बारे में पूछें...",
    suggested: "सुझाए गए विषय",
    tip: "आज का सुझाव",
    tipsList: [
        "फसल चक्रण सिंथेटिक उर्वरकों के बिना मिट्टी में पोषक तत्वों को वापस लाने में मदद करता है।",
        "वाष्पीकरण को कम करने के लिए सुबह जल्दी अपनी फसलों को पानी दें।",
        "मल्चिंग खरपतवार को दबाने और मिट्टी की नमी बनाए रखने में प्रभावी हो सकती है।",
        "इष्टतम पोषक तत्व अवशोषण सुनिश्चित करने के लिए सालाना अपनी मिट्टी के पीएच का परीक्षण करें।",
        "कीटों को स्वाभाविक रूप से नियंत्रित करने के लिए लेडीबग जैसे लाभकारी कीड़ों को प्रोत्साहित करें।",
        "ड्रिप सिंचाई बाढ़ सिंचाई की तुलना में 50% तक पानी बचाती है।",
        "जैविक कचरे की खाद लैंडफिल उपयोग को कम करती है और आपकी मिट्टी को समृद्ध करती है।"
    ],

    // Tour
    takeTour: "टूर लें",
    tourNext: "अगला",
    tourBack: "वापस",
    tourSkip: "छोड़ें",
    tourFinish: "समाप्त",
    tourStep1Title: "भाषा चयन",
    tourStep1Desc: "यहाँ से शुरू करें! बेहतर पहुंच के लिए एप्लिकेशन की भाषा को हिंदी, कन्नड़ या तेलुगु में बदलें।",
    tourStep2Title: "डैशबोर्ड",
    tourStep2Desc: "यह आपका घर है। अपने खेत की स्थिति, त्वरित आंकड़े और उपज अनुमानों का सारांश देखें।",
    tourStep3Title: "मौसम विजेट",
    tourStep3Desc: "वास्तविक समय के मौसम और गंभीर चेतावनियों की जाँच करें। अन्य स्थानों के लिए पूर्वानुमान देखने के लिए खोज पट्टी का उपयोग करें।",
    tourStep4Title: "फसल सलाहकार",
    tourStep4Desc: "रोपण से पहले इसका उपयोग करें। अपनी भूमि के लिए उपयुक्त एआई-अनुशंसित फसलें प्राप्त करने के लिए अपनी मिट्टी का डेटा दर्ज करें।",
    tourStep5Title: "उर्वरक सलाहकार",
    tourStep5Desc: "विकास के दौरान इसका उपयोग करें। मिट्टी को नुकसान पहुँचाए बिना उपज को अधिकतम करने के लिए कस्टम पोषक तत्व योजनाएँ प्राप्त करें।",
    tourStep6Title: "कीट नियंत्रण",
    tourStep6Desc: "यदि आपको कीड़े या बीमारी दिखाई दे तो इसका उपयोग करें। कीटों की पहचान करने और उपचार सलाह प्राप्त करने के लिए एक फोटो अपलोड करें।",
    tourStep7Title: "कृषि-चैट",
    tourStep7Desc: "कोई विशिष्ट प्रश्न है? खेती के बारे में त्वरित उत्तर के लिए हमारे एआई सहायक के साथ चैट करें।",
    tourStep8Title: "घर लौटें",
    tourStep8Desc: "मुख्य डैशबोर्ड पर लौटने के लिए किसी भी समय यहाँ क्लिक करें।"
  },
  kn: {
    dashboard: "ಡ್ಯಾಶ್‌ಬೋರ್ಡ್",
    cropAdvisor: "ಬೆಳೆ ಸಲಹೆಗಾರ",
    fertilizers: "ಗೊಬ್ಬರಗಳು",
    pestControl: "ಕೀಟ ನಿಯಂತ್ರಣ",
    chat: "ಕೃಷಿ-ಚಾಟ್",
    welcome: "ಮತ್ತೆ ಸ್ವಾಗತ,",
    farmer: "ರೈತ",
    yieldPred: "ಇಳುವರಿ ಮುನ್ಸೂಚನೆ",
    fieldReport: "ಕ್ಷೇತ್ರ ವರದಿ ವೀಕ್ಷಿಸಿ",
    totalYield: "ಒಟ್ಟು ಇಳುವರಿ",
    activeFields: "ಸಕ್ರಿಯ ಜಮೀನುಗಳು",
    equipStatus: "ಉಪಕರಣಗಳ ಸ್ಥಿತಿ",
    waterUsage: "ನೀರಿನ ಬಳಕೆ",
    yieldProj: "ಇಳುವರಿ ಯೋಜನೆ",
    soilMoisture: "ಮಣ್ಣಿನ ತೇವಾಂಶ ಮಟ್ಟಗಳು",
    searchCity: "ನಗರವನ್ನು ಹುಡುಕಿ...",
    weather: "ಹವಾಮಾನ",
    humidity: "ತೇವಾಂಶ",
    wind: "ಗಾಳಿ",
    uvIndex: "ಯುವಿ ಸೂಚ್ಯಂಕ",
    forecast: "5-ದಿನದ ಮುನ್ಸೂಚನೆ",

    // Crop Advisor
    soilData: "ಮಣ್ಣಿನ ಮಾಹಿತಿ",
    enterParams: "ನಿಮ್ಮ ಜಮೀನಿನ ವಿವರಗಳನ್ನು ನಮೂದಿಸಿ",
    nitrogen: "ಸಾರಜನಕ (N)",
    phosphorus: "ರಂಜಕ (P)",
    potassium: "ಪೊಟ್ಯಾಸಿಯಮ್ (K)",
    phLevel: "ಪಿಎಚ್ ಮಟ್ಟ",
    rainfall: "ಮಳೆ (ಮಿಮೀ)",
    analyze: "ವಿಶ್ಲೇಷಿಸಿ ಮತ್ತು ಶಿಫಾರಸು ಮಾಡಿ",
    analyzing: "ವಿಶ್ಲೇಷಿಸಲಾಗುತ್ತಿದೆ...",
    confidence: "ಭರವಸೆ",
    growthTime: "ಬೆಳವಣಿಗೆಯ ಸಮಯ",
    yield: "ಇಳುವರಿ",
    economy: "ಆರ್ಥಿಕತೆ",
    analysis: "ವಿಶ್ಲೇಷಣೆ",
    requirements: "ಅವಶ್ಯಕತೆಗಳು",
    waterNeeds: "ನೀರಿನ ಅವಶ್ಯಕತೆ",
    sunlight: "ಸೂರ್ಯನ ಬೆಳಕು",
    soilPref: "ಮಣ್ಣಿನ ಆದ್ಯತೆ",
    clickAnalysis: "ವಿಶ್ಲೇಷಣೆಗಾಗಿ ಕ್ಲಿಕ್ ಮಾಡಿ",
    noCrops: "ಕೃತಕ ಬುದ್ಧಿಮತ್ತೆ ಆಧಾರಿತ ಬೆಳೆ ಸಲಹೆ ಪಡೆಯಲು ಮಣ್ಣಿನ ವಿವರಗಳನ್ನು ನಮೂದಿಸಿ.",

    // Fertilizer
    calcTitle: "ಸ್ಮಾರ್ಟ್ ಗೊಬ್ಬರ ಕ್ಯಾಲ್ಕುಲೇಟರ್",
    calcDesc: "ನಿಮ್ಮ ನಿರ್ದಿಷ್ಟ ಬೆಳೆ ಮತ್ತು ಮಣ್ಣಿನ ಪ್ರಕಾರಕ್ಕೆ ತಕ್ಕಂತೆ ನಿಖರವಾದ ಪೋಷಕಾಂಶಗಳ ಯೋಜನೆ ಪಡೆಯಿರಿ.",
    targetCrop: "ಉದ್ದೇಶಿತ ಬೆಳೆ",
    soilCond: "ಮಣ್ಣಿನ ಸ್ಥಿತಿ",
    genPlan: "ಪೋಷಕಾಂಶಗಳ ಯೋಜನೆಯನ್ನು ರಚಿಸಿ",
    appFreq: "ಬಳಕೆಯ ಆವರ್ತನ",
    dosage: "ಶಿಫಾರಸು ಮಾಡಿದ ಪ್ರಮಾಣ",
    warnings: "ಪ್ರಮುಖ ಎಚ್ಚರಿಕೆಗಳು",
    loamy: "ಮೆಕ್ಕಲು ಮಣ್ಣು (ಸಮತೋಲಿತ)",
    clay: "ಜೇಡಿ ಮಣ್ಣು (ಭಾರ)",
    sandy: "ಮರಳು ಮಣ್ಣು (ಬೇಗನೆ ಬರಿದಾಗುತ್ತದೆ)",
    silt: "ವಂಡಲು ಮಣ್ಣು (ತೇವಾಂಶ ಉಳಿಸಿಕೊಳ್ಳುತ್ತದೆ)",
    peaty: "ಪೀಟಿ (ಆಮ್ಲೀಯ)",

    // Pest
    pestTitle: "ಎಐ ಕೀಟ ರೋಗನಿರ್ಣಯ",
    pestDesc: "ಬಾಧಿತ ಸಸ್ಯದ ಎಲೆ ಅಥವಾ ಕೀಟದ ಫೋಟೋವನ್ನು ತ್ವರಿತ ವಿಶ್ಲೇಷಣೆಗಾಗಿ ಅಪ್‌ಲೋಡ್ ಮಾಡಿ.",
    upload: "ಅಪ್‌ಲೋಡ್ ಮಾಡಲು ಕ್ಲಿಕ್ ಮಾಡಿ",
    changePhoto: "ಫೋಟೋ ಬದಲಾಯಿಸಿ",
    diagnose: "ಸಮಸ್ಯೆಯನ್ನು ಗುರುತಿಸಿ",
    analyzingPest: "ಎಐ ಚಿತ್ರವನ್ನು ಪರಿಶೀಲಿಸುತ್ತಿದೆ...",
    treatments: "ಸೂಚಿಸಲಾದ ಚಿಕಿತ್ಸೆಗಳು",
    prevention: "ತಡೆಗಟ್ಟುವ ಕ್ರಮಗಳು",
    risk: "ಅಪಾಯ",

    // Chat
    chatTitle: "ಕೃಷಿ ಸಲಹೆಗಾರ",
    online: "ಆನ್‌ಲೈನ್",
    clearChat: "ಚಾಟ್ ಅಳಿಸಿ",
    typeMsg: "ಬೆಳೆಗಳು, ಕೀಟಗಳು ಅಥವಾ ಕೃಷಿ ಸಲಹೆಗಳ ಬಗ್ಗೆ ಕೇಳಿ...",
    suggested: "ಸೂಚಿಸಲಾದ ವಿಷಯಗಳು",
    tip: "ಇಂದಿನ ಸಲಹೆ",
    tipsList: [
        "ಬೆಳೆ ಬದಲಾವಣೆ ಮಾಡುವುದರಿಂದ ರಾಸಾಯನಿಕ ಗೊಬ್ಬರಗಳಿಲ್ಲದೆ ಮಣ್ಣಿಗೆ ಪೋಷಕಾಂಶಗಳನ್ನು ಹಿಂದಿರುಗಿಸಲು ಸಹಾಯವಾಗುತ್ತದೆ.",
        "ಆவியಾಗುವಿಕೆಯನ್ನು ಕಡಿಮೆ ಮಾಡಲು ಬೆಳಿಗ್ಗೆ ಬೇಗನೆ ನಿಮ್ಮ ಬೆಳೆಗಳಿಗೆ ನೀರು ಹಾಕಿ.",
        "ಮಲ್ಚಿಂಗ್ ಕಳೆಗಳನ್ನು ತಡೆಯಲು ಮತ್ತು ಮಣ್ಣಿನ ತೇವಾಂಶವನ್ನು ಉಳಿಸಿಕೊಳ್ಳಲು ಪರಿಣಾಮಕಾರಿಯಾಗಿದೆ.",
        "ಸರಿಯಾದ ಪೋಷಕಾಂಶಗಳ ಹೀರಿಕೊಳ್ಳುವಿಕೆಯನ್ನು ಖಚಿತಪಡಿಸಿಕೊಳ್ಳಲು ವಾರ್ಷಿಕವಾಗಿ ಮಣ್ಣಿನ ಪಿಎಚ್ ಪರೀಕ್ಷಿಸಿ.",
        "ಕೀಟಗಳನ್ನು ನೈಸರ್ಗಿಕವಾಗಿ ನಿಯಂತ್ರಿಸಲು ಲೇಡಿಬಗ್‌ಗಳಂತಹ ಹಿತಕಾರಿ ಕೀಟಗಳನ್ನು ಪ್ರೋತ್ಸಾಹಿಸಿ.",
        "ಹನಿ ನೀರಾವರಿ ಪ್ರವಾಹ ನೀರಾವರಿಗೆ ಹೋಲಿಸಿದರೆ 50% ನೀರನ್ನು ಉಳಿಸುತ್ತದೆ.",
        "ಸಾವಯವ ತ್ಯಾಜ್ಯದಿಂದ ಗೊಬ್ಬರ ಮಾಡುವುದರಿಂದ ತ್ಯಾಜ್ಯವನ್ನು ಕಡಿಮೆ ಮಾಡುತ್ತದೆ ಮತ್ತು ಮಣ್ಣನ್ನು ಫಲವತ್ತಾಗಿಸುತ್ತದೆ."
    ],

    // Tour
    takeTour: "ಪ್ರವಾಸ ಕೈಗೊಳ್ಳಿ",
    tourNext: "ಮುಂದೆ",
    tourBack: "ಹಿಂದೆ",
    tourSkip: "ಸ್ಕಿಪ್ ಮಾಡಿ",
    tourFinish: "ಮುಗಿಸಿ",
    tourStep1Title: "ಭಾಷಾ ಆಯ್ಕೆ",
    tourStep1Desc: "ಇಲ್ಲಿ ಪ್ರಾರಂಭಿಸಿ! ಉತ್ತಮ ಪ್ರವೇಶಕ್ಕಾಗಿ ಅಪ್ಲಿಕೇಶನ್ ಭಾಷೆಯನ್ನು ಹಿಂದಿ, ಕನ್ನಡ ಅಥವಾ ತೆಲುಗಿಗೆ ಬದಲಾಯಿಸಿ.",
    tourStep2Title: "ಡ್ಯಾಶ್‌ಬೋರ್ಡ್",
    tourStep2Desc: "ಇದು ನಿಮ್ಮ ಮುಖಪುಟ. ನಿಮ್ಮ ಕೃಷಿ ಸ್ಥಿತಿ, ತ್ವರಿತ ಅಂಕಿಅಂಶಗಳು ಮತ್ತು ಇಳುವರಿ ಅಂದಾಜುಗಳನ್ನು ವೀಕ್ಷಿಸಿ.",
    tourStep3Title: "ಹವಾಮಾನ ವಿಜೆಟ್",
    tourStep3Desc: "ನೈಜ-ಸಮಯದ ಹವಾಮಾನ ಮತ್ತು ಗಂಭೀರ ಎಚ್ಚರಿಕೆಗಳನ್ನು ಪರಿಶೀಲಿಸಿ. ಇತರ ಸ್ಥಳಗಳಿಗೆ ಮುನ್ಸೂಚನೆಗಳನ್ನು ನೋಡಲು ಹುಡುಕಾಟ ಪಟ್ಟಿಯನ್ನು ಬಳಸಿ.",
    tourStep4Title: "ಬೆಳೆ ಸಲಹೆಗಾರ",
    tourStep4Desc: "ನಾಟಿ ಮಾಡುವ ಮೊದಲು ಇದನ್ನು ಬಳಸಿ. ನಿಮ್ಮ ಭೂಮಿಗೆ ಸೂಕ್ತವಾದ ಎಐ-ಶಿಫಾರಸು ಮಾಡಿದ ಬೆಳೆಗಳನ್ನು ಪಡೆಯಲು ನಿಮ್ಮ ಮಣ್ಣಿನ ಡೇಟಾವನ್ನು ನಮೂದಿಸಿ.",
    tourStep5Title: "ಗೊಬ್ಬರ ಸಲಹೆಗಾರ",
    tourStep5Desc: "ಬೆಳವಣಿಗೆಯ ಸಮಯದಲ್ಲಿ ಇದನ್ನು ಬಳಸಿ. ಮಣ್ಣಿಗೆ ಹಾನಿಯಾಗದಂತೆ ಇಳುವರಿಯನ್ನು ಹೆಚ್ಚಿಸಲು ಕಸ್ಟಮ್ ಪೋಷಕಾಂಶಗಳ ಯೋಜನೆಗಳನ್ನು ಪಡೆಯಿರಿ.",
    tourStep6Title: "ಕೀಟ ನಿಯಂತ್ರ",
    tourStep6Desc: "ನೀವು ಕೀಟಗಳು ಅಥವಾ ರೋಗವನ್ನು ಕಂಡರೆ ಇದನ್ನು ಬಳಸಿ. ಕೀಟಗಳನ್ನು ಗುರುತಿಸಲು ಮತ್ತು ಚಿಕಿತ್ಸಾ ಸಲಹೆಯನ್ನು ಪಡೆಯಲು ಫೋಟೋ ಅಪ್‌ಲೋಡ್ ಮಾಡಿ.",
    tourStep7Title: "ಕೃಷಿ-ಚಾಟ್",
    tourStep7Desc: "ನಿರ್ದಿಷ್ಟ ಪ್ರಶ್ನೆ ಇದೆಯೇ? ಕೃಷಿಯ ಬಗ್ಗೆ ತ್ವರಿತ ಉತ್ತರಗಳಿಗಾಗಿ ನಮ್ಮ ಎಐ ಸಹಾಯಕನೊಂದಿಗೆ ಚಾಟ್ ಮಾಡಿ.",
    tourStep8Title: "ಮುಖಪುಟಕ್ಕೆ ಹಿಂತಿರುಗಿ",
    tourStep8Desc: "ಮುಖ್ಯ ಡ್ಯಾಶ್‌ಬೋರ್ಡ್‌ಗೆ ಹಿಂತಿರುಗಲು ಯಾವಾಗ ಬೇಕಾದರೂ ಇಲ್ಲಿ ಕ್ಲಿಕ್ ಮಾಡಿ."
  },
  te: {
    dashboard: "డ్యాష్‌బోర్డ్",
    cropAdvisor: "పంట సలహాదారు",
    fertilizers: "ఎరువులు",
    pestControl: "చీడపీడల నివారణ",
    chat: "అగ్రి-చాట్",
    welcome: "స్వాగతం,",
    farmer: "రైతు",
    yieldPred: "దిగుబడి అంచనా",
    fieldReport: "క్షేత్ర నివేదికను చూడండి",
    totalYield: "మొత్తం దిగుబడి",
    activeFields: "సాగులో ఉన్న పొలాలు",
    equipStatus: "పరికరాల స్థితి",
    waterUsage: "నీటి వినియోగం",
    yieldProj: "దిగుబడి ప్రొజెక్షన్",
    soilMoisture: "నేల తేమ స్థాయిలు",
    searchCity: "నగరాన్ని వెతకండి...",
    weather: "వాతావరణం",
    humidity: "తేమ",
    wind: "గాలి",
    uvIndex: "UV సూచిక",
    forecast: "5-రోజుల సూచన",

    // Crop Advisor
    soilData: "నేల సమాచారం",
    enterParams: "మీ పొలం వివరాలను నమోదు చేయండి",
    nitrogen: "నైట్రోజన్ (N)",
    phosphorus: "ఫాస్ఫరస్ (P)",
    potassium: "పొటాషియం (K)",
    phLevel: "pH స్థాయి",
    rainfall: "వర్షపాతం (mm)",
    analyze: "విశ్లేషించండి & సిఫార్సు చేయండి",
    analyzing: "విశ్లేషిస్తోంది...",
    confidence: "నమ్మకం",
    growthTime: "పెరుగుదల సమయం",
    yield: "దిగుబడి",
    economy: "ఆర్థిక వ్యవస్థ",
    analysis: "విశ్లేషణ",
    requirements: "అవసరాలు",
    waterNeeds: "నీటి అవసరాలు",
    sunlight: "సూర్యకాంతి",
    soilPref: "నేల రకం",
    clickAnalysis: "విశ్లేషణ కోసం క్లిక్ చేయండి",
    noCrops: "AI ఆధారిత పంట సలహాలను పొందడానికి నేల వివరాలను నమోదు చేయండి.",

    // Fertilizer
    calcTitle: "స్మార్ట్ ఎరువుల కాలిక్యులేటర్",
    calcDesc: "మీ నిర్దిష్ట పంట మరియు నేల రకానికి తగిన పోషక మోతాదు ప్రణాళికను పొందండి.",
    targetCrop: "లక్ష్య పంట",
    soilCond: "నేల పరిస్థితి",
    genPlan: "పోషక ప్రణాళికను రూపొందించండి",
    appFreq: "అప్లికేషన్ తరచుదనం",
    dosage: "సిఫార్సు మోతాదు",
    warnings: "ముఖ్యమైన హెచ్చరికలు",
    loamy: "లోమీ (సమతుల్య)",
    clay: "బంకమట్టి (బరువు)",
    sandy: "ఇసుక (త్వరగా ఎండిపోతుంది)",
    silt: "సిల్ట్ (తేమను నిలుపుకుంటుంది)",
    peaty: "పీటీ (ఆమ్ల)",

    // Pest
    pestTitle: "AI తెగులు నిర్ధారణ",
    pestDesc: "తక్షణ విశ్లేషణ కోసం ప్రభావిత మొక్క ఆకు లేదా పురుగు ఫోటోను అప్‌లోడ్ చేయండి.",
    upload: "అప్‌లోడ్ చేయడానికి క్లిక్ చేయండి",
    changePhoto: "ఫోటో మార్చండి",
    diagnose: "సమస్యను గుర్తించండి",
    analyzingPest: "AI చిత్రాన్ని పరిశీలిస్తోంది...",
    treatments: "సూచించిన చికిత్సలు",
    prevention: "నివారణ చర్యలు",
    risk: "ప్రమాదం",

    // Chat
    chatTitle: "వ్యవసాయ సలహాదారు",
    online: "ఆన్‌లైన్",
    clearChat: "చాಟ್ క్లియర్ చేయండి",
    typeMsg: "పంటలు, తెగుళ్లు లేదా వ్యవసాయ చిట్కాల గురించి అడగండి...",
    suggested: "సూచించిన అంశాలు",
    tip: "నేటి చిట్కా",
    tipsList: [
        "పంట మార్పిడి సింథటిక్ ఎరువులు లేకుండా నేలకు పోషకాలను తిరిగి ఇవ్వడానికి సహాయపడుతుంది.",
        "బాష్పీభవనాన్ని తగ్గించడానికి ఉదయాన్నే మీ పంటలకు నీరు పెట్టండి.",
        "మల్చింగ్ కలుపు మొక్కలను అణచివేయడానికి మరియు నేల తేమను నిలుపుకోవడానికి సమర్థవంతంగా పనిచేస్తుంది.",
        "సరైన పోషక శోషణను నిర్ధారించడానికి ఏటా మీ నేల pH ని పరీక్షించండి.",
        "తెగుళ్లను సహజంగా నియంత్రించడానికి లేడీబగ్స్ వంటి మేలు చేసే కీటకాలను ప్రోత్సహించండి.",
        "వరద నీటిపారుదలతో పోలిస్తే బిందు సేద్యం 50% వరకు నీటిని ఆదా చేస్తుంది.",
        "సేంద్రీయ వ్యర్థాలను కంపోస్ట్ చేయడం వలన ల్యాండ్‌ఫిల్ వినియోగాన్ని తగ్గిస్తుంది మరియు మీ నేలను సారవంతం చేస్తుంది."
    ],

    // Tour
    takeTour: "టూర్ తీసుకోండి",
    tourNext: "తరువాత",
    tourBack: "వెనుకకు",
    tourSkip: "వదిలివేయి",
    tourFinish: "ముగించు",
    tourStep1Title: "భాష ఎంపిక",
    tourStep1Desc: "ఇక్కడ ప్రారంభించండి! మెరుగైన ప్రాప్యత కోసం అప్లికేషన్ భాషను హిందీ, కన్నడ లేదా తెలుగుకు మార్చండి.",
    tourStep2Title: "డ్యాష్‌బోర్డ్",
    tourStep2Desc: "ఇది మీ హోమ్. మీ పొలం స్థితి, త్వరిత గణాంకాలు మరియు దిగుబడి అంచనాల సారాంశాన్ని వీక్షించండి.",
    tourStep3Title: "వాతావరణ విడ్జెట్",
    tourStep3Desc: "నిజ-సమయ వాతావరణం మరియు తీవ్రమైన హెచ్చరికలను తనిఖీ చేయండి. ఇతర ప్రాంతాల కోసం సూచనలను చూడటానికి శోధన పట్టీని ఉపయోగించండి.",
    tourStep4Title: "పంట సలహాదారు",
    tourStep4Desc: "నాటడానికి ముందు దీన్ని ఉపయోగించండి. మీ భూమికి తగిన AI-సిఫార్సు చేసిన పంటలను పొందడానికి మీ నేల డేటాను నమోదు చేయండి.",
    tourStep5Title: "ఎరువుల సలహాదారు",
    tourStep5Desc: "పెరుగుదల సమయంలో దీన్ని ఉపయోగించండి. నేలకు హాని కలిగించకుండా దిగుబడిని పెంచడానికి అనుకూల పోషక ప్రణాళికలను పొందండి.",
    tourStep6Title: "చీడపీడల నివారణ",
    tourStep6Desc: "మీరు పురుగులు లేదా వ్యాధిని చూస్తే దీన్ని ఉపయోగించండి. తెగుళ్లను గుర్తించడానికి మరియు చికిత్స సలహాలను పొందడానికి ఫోటోను అప్‌లోడ్ చేయండి.",
    tourStep7Title: "అగ్రి-చాట్",
    tourStep7Desc: "నిర్దిష్ట ప్రశ్న ఉందా? వ్యవసాయం గురించి తక్షణ సమాధానాల కోసం మా AI అసిస్టెంట్‌తో చాట్ చేయండి.",
    tourStep8Title: "హోమ్‌కు తిరిగి వెళ్ళు",
    tourStep8Desc: "ప్రధాన డ్యాష్‌బోర్డ్‌కు తిరిగి వెళ్లడానికి ఎప్పుడైనా ఇక్కడ క్లిక్ చేయండి."
  }
};

export const getTranslation = (lang: Language) => TRANSLATIONS[lang];
