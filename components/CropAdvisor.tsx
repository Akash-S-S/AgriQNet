...
const CropAdvisor: React.FC<CropAdvisorProps> = ({ lang }) => {
  ...
  const handleRecommend = async () => {
    setLoading(true);
    try {
      const results = await GeminiService.recommendCrops(soilData, lang);
  ...
  const getCropImage = (cropName: string) => {
    // Note: Since cropName might now be in a different language, 
    // we should try to match English keys if possible, or fallback to loremflickr.
    // For specific translations, the logic below mainly works on the text provided.
    // Ideally, the API would return an english ID, but here we do best-effort.
    const normalizedName = cropName.toLowerCase().trim();
    
    // Check strict match against keys
    ...
    // Fallback to dynamic service if not found
        ...
        <div className="w-full md:w-1/3 space-y-6">
          <div className="bg-white rounded-3xl p-6 shadow-lg border border-agri-100">
            <div className="flex items-center gap-3 mb-6">
              ...
              <div>
                <h2 className="text-xl font-bold text-gray-800">{t.soilData}</h2>
                <p className="text-sm text-gray-500">{t.enterParams}</p>
              </div>
            </div>
